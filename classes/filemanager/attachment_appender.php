...
class attachment_appender {
    ...
    private function add_download_all_links()
    {
        if ($this->has_attachments()) {
            $this->body .= $this->hr();
            $this->body .= block_quickmail_plugin::_s('moodle_attachments', $this->get_download_all_link());
            $this->body .= $this->br();
    ...
    private function get_download_all_link($as_url = false)
    {
        ...
        return ! $as_url
            ? html_writer::link($url, block_quickmail_plugin::_s('download_all'))
    ...
    private function add_individual_files()
    {
        if ($this->has_attachments()) {
            $this->body .= $this->hr();
            $this->body .= block_quickmail_plugin::_s('qm_contents');
            $this->body .= $this->br();
